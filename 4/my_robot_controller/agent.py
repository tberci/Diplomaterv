...
class MyNode(Node):
    ...
    def __init__(self):
        ...
        self.position_pub = self.create_publisher(Twist,"agent"+self.value+"/position",10) # ágens pozicio
        ...
        self.cmd_vel_sub = self.create_subscription(Twist, "agent"+self.value+"/cmd_vel",self.listener_callback,10)
   ...
   #     self.x = msg.linear.x
    ...
    def agent_velocity_pub(self):
        ...
        msg.linear.x = random.uniform(-4.0, -3.0)
        msg.linear.y = 0.0
        msg.linear.z = 0.0
        ...
        msg.angular.x = 0.0
        msg.angular.y = 0.0
        msg.angular.z = random.uniform(-5.0, -0.5)
